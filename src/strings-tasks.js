...
function formatTime(minutes, seconds) {
  const min = minutes.toFixed().padStart(2, '0');
  const sec = seconds.toFixed().padStart(2, '0');
...
function countVowels(str) {
  return [...str].reduce(
    (acc, char) => ('aeiouy'.includes(char.toLowerCase()) ? acc + 1 : acc),
    0
  );
}
...
function isPalindrome(str) {
  const newStr = [...str.toLowerCase()].filter((char) =>
...
function unbracketTag(str) {
  return [...str].filter((char) => char !== '<' && char !== '>').join('');
}
...
function encodeToRot13(str) {
  ...
  const string = [...str].reduce((rot13, char) => {
    const index = alphabetLC.indexOf(char.toLowerCase());

    if (index !== -1) {
      const newIndex = (index + 13) % 26;
      const newChar = alphabetLC.includes(char)
        ? alphabetLC[newIndex]
        : alphabetLC[newIndex].toUpperCase();
      return rot13 + newChar;
    }
    return rot13 + char;
  }, '');
...
function getCardId(value) {
  ...
  return ranks.indexOf(num) + 13 * suits.indexOf(card);
}
